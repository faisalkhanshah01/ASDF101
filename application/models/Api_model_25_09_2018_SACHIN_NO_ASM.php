		...
		if(!empty($_REQUEST['email'])  && !empty($_REQUEST['password'])){
		 ...
		 if(!empty($_REQUEST['from']) && ($_REQUEST['from'] == 'ios')){
			if(isset($_REQUEST['user_img'])){
					$user_arr = explode('##', $_REQUEST['user_img']);
					$ext = strtolower($user_arr[1]);
					$img = base64_decode($user_arr[0]);
					$image_name = $img.$ext;
					...
					$file 		= $folder_path . $fileName;
					$fileType 	= 'image/'.$ext;
					$filePath	= "./uploads/images/users/";
					
					$success 	= file_put_contents($file, $image_name);
					//print_r($ext);print '<br/>';
					if($success){
						//$user_img =	"/uploads/images/users/".$fileName;
						$user_img =	$fileName;
					}else{
						$user_img = '';
					}
	...
	*/
	...
	public function verify_user_login_new($identity,$verify_password,$imei_no){
		...
		if($this->verify_password($identity,$verify_password)){
			...
			if(($checkIMEI > 0) && ($checkIMEI['upro_mob_imei'] == $imei_no)){
				...
				return $user = array('email'=>$identity,'user_fullName'=>$user_fullName, 'userID'=>$user['uacc_id'],'userGroupID'=>$user['uacc_group_fk'], 'userType'=>$user['ugrp_name'],'imei_no'=>$user['upro_mob_imei'],'authenticaton'=>1,'error'=>'');
			}else{
	...
	public function verify_user_login($identity, $verify_password)
	{
		...
		if($this->verify_password($identity,$verify_password)){
			...
			$user=$result->row_array();
			$user_fullName = $user['upro_first_name'].' '.$user['upro_last_name'];
			return $user = array('email'=>$identity,'user_fullName'=>$user_fullName, 'userID'=>$user['uacc_id'],'userGroupID'=>$user['uacc_group_fk'], 'userType'=>$user['ugrp_name'],'authenticaton'=>1,'error'=>'');
		}else{
			//return false;
		return $error = array('email'=>$identity,'authenticaton'=>0,'user_fullName'=>'', 'userID'=>'','userGroupID'=>'', 'userType'=>'','error'=>"Username or Password  is Incorrect");	
		}
    ...
    public function frequency_month_mdata_dates($DatesArray){
		...
		$mdata_material_invoice_date	= ( ($DatesArray['mdata_material_invoice_date'] == '1970-01-01' ) || ($DatesArray['mdata_material_invoice_date'] == '000-00-00' )  ) ? '' : $DatesArray['mdata_material_invoice_date'] ;

		$date_of_first_use				= ( ($DatesArray['date_of_first_use'] == '1970-01-01' ) || ($DatesArray['date_of_first_use'] == '000-00-00' )  ) ? '' : $DatesArray['date_of_first_use'] ;
		$date_of_inspection				= ( ($DatesArray['date_of_inspection'] == '1970-01-01' ) || ($DatesArray['date_of_inspection'] == '000-00-00' )  ) ? '' : $DatesArray['date_of_inspection'] ;
		$inspection_due_date			= ( ($DatesArray['inspection_due_date'] == '1970-01-01' ) || ($DatesArray['inspection_due_date'] == '000-00-00' )  ) ? '' : $DatesArray['inspection_due_date'] ;
