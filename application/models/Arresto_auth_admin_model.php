...
class Arresto_auth_admin_model extends CI_Model {
	 ...
	 */
        ...
        function u1(){
           ...
           $this->db->select("*");
           $query=$this->db->get('user_accounts');   // return result set object 
           if($query->num_rows()>0){
	...
	{
	   # arresto clients listing 
           $sql="select * from ar_clients_2018 as client join user_accounts as user on client.customer_uacc_fk=user.uacc_id";
           $query=$this->db->query($sql);
           $result=array();
           if($query->num_rows($sql)){
       ...
       function update_account_new($client_id)
	{
...
//				'upro_newsletter' => ''
			...
			$profile_data['emp_region'] = (isset($_POST['emp_region']))? $this->input->post('emp_region') : '';
                         ...
                         $client_info = array(
                            ...
                            'customer_theme_color'=>$this->input->post('register_theme_color')
			);
                        ...
                        if($query->num_rows()){
                            $r=$query->row_array();
                            $user_id=$r['customer_uacc_fk'];
                            
                            // update client_2018 table
                            $this->db->where('customer_id',$client_id);
                            $this->db->update('ar_clients_2018',$client_info);
                            $this->db->last_query();
	...
	function get_user_accounts_block($group_id=null)
	{
		# get parent 
...
*/
       ...
       #get user   
		...
		if (! $this->flexi_auth->in_group('Master Admin'))
		{
			...
			$sql_where[$this->flexi_auth->db_column('user_group', 'id').'='] = 1;   # only ROOT USER WILL BE DISPLAYED
			//$this->flexi_auth->sql_where($sql_where);
		}	
        


		// get the ROOT CHILD GROUPS 
		//$child_groups = $this->flexi_auth->get_groups("",array('uprp_pid'=>1));
	...
	*/
	...
	function insert_user_group()
	{
		...
		if ($this->form_validation->run())
		{
			...
			$this->flexi_auth->insert_group($group_name,$group_desc,$group_admin,array('ugrp_pid'=>$parent_group_id));
