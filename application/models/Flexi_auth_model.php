...
class Flexi_auth_model extends Flexi_auth_lite_model
{
    ...
    function fetch_kdmanger_mailId($kdMangerID){
        if(!empty($kdMangerID)){
			//SELECT * FROM `user_accounts` WHERE `uacc_group_fk` = 10 
			$this->db->select('uacc_email');
			$this->db->where('uacc_group_fk',addslashes($kdMangerID));
	...
	function get_user_name($userID){
		$this->db->select('*');
		$this->db->where('upro_uacc_fk',addslashes($userID));
		$query=$this->db->get('demo_user_profiles');
		if($query->num_rows()>0){
			$return = $query->row_array();
			return $return['upro_first_name'] .' '.$return['upro_last_name'];
		}else{
			//return "NO data";
			return false;
		}
	...
	/***********************admin change password********************************/
         function update_passwordAdmin($param){
            if(!empty($param['emailID']) && !empty($param['new_password']) && !empty($param['userID'])){
               
                $sql_select = $this->auth->tbl_col_user_account['salt'];
                $sql_where = array(
                        $this->auth->primary_identity_col => $param['emailID']
                );
                $user = $this->get_users($sql_select, $sql_where)->row();
              
                $hash_new_password = $this->generate_hash_token($param['new_password'], $user->{$this->auth->database_config['user_acc']['columns']['salt']}, TRUE);
               
                $sql_update[$this->auth->tbl_col_user_account['password']] = $hash_new_password;
                $sql_where[$this->auth->primary_identity_col] = $param['emailID'];
                $return =  $this->db->update($this->auth->tbl_user_account, $sql_update, $sql_where);
                if($return > 0){
                    return 1;
                }else{
                    return -1;
	...
	{	
		...
		$user = $query->row();
                ...
                // if user is not register for current domain via sachin 
                if($user->uacc_client_fk && $user->uacc_client_fk!==$_SESSION['client']['client_id']){
                     $this->set_error_message('User is not Registered with this Domian');
		     		 return FALSE;   
                }
                // if login user is client 
                if($user->uacc_client_fk==0 && $user->uacc_group_fk==3){
                    $this->load->model('build_model');
                    // get the root admin client 
                    $client_group = $this->build_model->get_data_row('client_groups',array('cgrp_client_fk'=>$user->uacc_id,'cgrp_pid'=>1));
                    #echo "<pre>";print_r($client_group); echo$_SESSION['client']['client_grpid']; die;  
                    if($client_group){
                    	if($client_group['cgrp_id']!=$_SESSION['client']['client_grpid']){
                    		  $this->set_error_message('User is not Registered with this Domian');
		     		 		  return FALSE;   
                    	   }
			...
			if ($this->verify_password($identity, $password))
			{
				...
				/* Edited by Shakti Singh // To get logged in user image name in session. */
					$user_details = $this->flexi_auth->get_user_by_id_row_array($user->uacc_id);
					...
					$data = array(
									'firstName' => (! empty($user_details)) ? $user_details['upro_first_name'] : null,
									'lastName'  => (! empty($user_details)) ? $user_details['upro_last_name'] : null,
									'userImage' => (! empty($user_details)) ?  $user_details['upro_image'] : null,
									'language'  => $user_lang
									);
	 ...
	 * @author Rob Hussey
	 */
	public function send_email($email_to = NULL,$email_title = NULL, $data = NULL, $template = NULL, $attachment = '')
	//public function send_email($email_to = NULL,$email_title = NULL, $data = NULL, $template = NULL)
	{
		...
		*/
		/*
			Code changed by Shakti Singh on 24-11-2015
		...
		*/
		...
		$email_config = array(
			...
			'charset' => $this->auth->email_settings['charset'],
			'wordwrap' => $this->auth->email_settings['wordwrap'],
			'newline' => $this->auth->email_settings['newline']
		);
		...
		if($this->auth->email_settings['bcc_emails']){
			//echo $this->auth->email_settings['bcc_emails'];
			$this->email->bcc($this->auth->email_settings['bcc_emails']);
		}

		$this->email->subject($this->auth->email_settings['site_title'] ." ". $email_title);
		$this->email->message($message);
		if($attachment !=''){
			// for multiple attachment or single attachment.
			if(is_array($attachment)){
				foreach($attachment as $attachVal){
					$this->email->attach($attachVal);
				}
			}else{
		...
		// If debug option is enabled print out the debug info
	 ...
	 */
	/* 
	public function insert_project_master_data($data, $custom_data = array())
  	{
		if (empty($name))
		{
			return FALSE;
