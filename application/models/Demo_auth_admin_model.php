...
class Demo_auth_admin_model extends CI_Model {
      ...
      function __construct(){
          ...
          if(count($this->session->userdata('client'))){
              $this->client = $_SESSION['client'];
              $this->base_url= base_url().$_SESSION['client']['url_slug']."/";
              $this->client_url= base_url().$_SESSION['client']['url_slug']."/";
          }
      }
	 ...
	 */
     ...
     function get_user_accounts(){
            ...
            // get the current loggedin user group 
            $group_id=$_SESSION['client']['group_id'];
            $client_id=$_SESSION['client']['client_id'];
            
             $sql="select users.uacc_id,users.uacc_group_fk,users.uacc_cgroup_fk,users.uacc_email,users.uacc_username,profiles.upro_first_name,profiles.upro_last_name,client_groups.cgrp_name,user_groups.ugrp_name from user_accounts as users 
                join demo_user_profiles as profiles on users.uacc_id=profiles.upro_uacc_fk 
                join client_groups on client_groups.cgrp_id=users.uacc_cgroup_fk 
                join user_groups on user_groups.ugrp_id=users.uacc_group_fk";
              $sql.= " where users.uacc_client_fk=$client_id";
              $result=array();
	...
	function get_user_accounts_block_by_sachin($group_id=null)
	{
            
           # echo "into this function"; die;
          
		# get parent 
        /*$group_id= $this->flexi_auth->get_user_group_id();
        $current_group=	$this->flexi_auth->get_groups_row_array("",array('ugrp_id'=>$group_id));
        $parent_group_id=$current_group['ugrp_pid'];

        #get child groups 
        #$child_groups =	$this->flexi_auth->get_groups_result_array("",array('ugrp_pid'=>$parent_group_id));
        $child_groups =	$this->flexi_auth->get_groups("ugrp_id",array('ugrp_pid'=>$parent_group_id))->result_array();


        $child_grps=array_column($child_groups,'ugrp_id');
        $child_grps[]= $parent_group_id;

      	$sql_where_in=$child_grps;
      
       */ 

        #$child_grps_str=implode(",", $child_grps); 
...
*/
       ...
       #get user   
		...
		$this->flexi_auth->sql_select($sql_select);
               # $this->flexi_auth->sql_where_in('uacc_id',$sql_where_in);
                
                #$user_clientId=$this->session->userdata('client['group_id']')
                
                $user_clientid=$_SESSION['client']['client_id'];
                $sql_where['user_accounts.uacc_cgroup_fk'] = $_SESSION['client']['group_id'];
                $sql_where['user_accounts.uacc_client_fk'] = $user_clientid;
		...
		if (! $this->flexi_auth->in_group('Master Admin'))
		{
                    $sql_where[$this->flexi_auth->db_column('user_group', 'id').' !='] = 3;                    
                     # only ROOT USER WILL BE DISPLAYED
                    $this->flexi_auth->sql_where($sql_where);
		}	
	...
	function update_user_account($user_id)
	{
		...
		$validation_rules = array(
			...
			array('field' => 'update_group', 'label' => 'User Group(Role)', 'rules' => 'required|integer'),
                       array('field' => 'update_cgroup', 'label' => 'User Group', 'rules' => 'required|integer')
		);
		...
		if ($this->form_validation->run())
		{
			...
			$profile_data = array(
				...
				'upro_newsletter' => $this->input->post('update_newsletter'),
                                ...
                                'uacc_cgroup_fk'=>$this->input->post('update_cgroup')
			);
//                        echo "<pre>";
...
//                        die;
			...
			if($this->input->post('update_group') != $_SESSION['user_account']['previous_user_group']){
                            
                            
			     /*foreach($_SESSION['user_account']['user_groups'] as $group){
					if($group['ugrp_id'] == $this->input->post('update_group')){
						if($group['ugrp_id'] =='2'){
							$groupName = 'Admin';
						}else{
				...
				// Send Mail to the User that You have been assigned as this particular group.
				//echo "Group Changed";
				$fullName 	= $this->input->post('update_first_name').' '.$this->input->post('update_last_name');
				$email 		= $this->input->post('update_email_address');
				$groupID 	= $this->input->post('update_group');
				$email_data = array(
									'full_name' => $fullName,
									'email' 	=> $email,
									'groupName' => $groupName,
				...
				//$email_data = array('identity' =>  $this->input->post('update_email_address'));
				
				$this->flexi_auth->send_email($email, 'New Group Assigned', 'registration_welcome.tpl.php', $email_data);
			
			}
			
			// Delete the session values of user_account.
			...
			// Redirect user.
			redirect($this->client_url.'auth_admin/manage_user_accounts');			
		}
	...
	/**
		* Display user Groups to Master Admin and Admin
		* No other user can See Master Admin Group
		* Created by Shakti Singh on 26-Nov-2015
	*/
	function display_manage_user_groups()
    {
...
//        $parent_group_id=$current_group['ugrp_pid'];
         ...
         $parent_group_id=$this->client['client_grpid'];
		...
		if (! $this->flexi_auth->in_group('Master Admin'))
		{
		  $sql_where[$this->flexi_auth->db_column('user_group', 'id').' !='] = 3;
                ...
                $sql_where['ugrp_pid']=$parent_group_id;
                
                
                
		$usergroup = $this->flexi_auth->sql_where($sql_where);
		$this->data['user_groups'] = $this->flexi_auth->get_groups_array($usergroup);
       ...
       function get_data($parent,&$output){
            static $data=[];
            $groups = $this->flexi_auth->get_groups("",array('ugrp_pid'=>$parent))->result_array();
            if(count($groups)){
                $data['key_'.$parent]=$groups;
            }
           
           foreach($groups as $group){
                 $this->get_data($group['ugrp_id'],$output);  
           }
   ...
   function manage_user_groups()
    {
		...
		if ($delete_groups = $this->input->post('delete_group'))
		{
			foreach($delete_groups as $group_id => $delete)
			{
				...
				if($group_id == 3 || $group_id == 2 || $group_id == 1){
					
					$this->flexi_auth_model->set_error_message('delete_unsuccessful', 'config');
					$data = 'You cannot delete this Default User Group';
	 ...
	 */
		...
		$this->load->library('form_validation');
                ...
                $parent_group_id=$this->client['group_id'];  // current loggend in client group id 
		// Set validation rules.
		$validation_rules = array(
		...
		if ($this->form_validation->run())
		{
			...
			$group_desc = $this->input->post('insert_group_description');
                        $group_parent = $this->input->post('insert_group_parent'); 
                        $group_parent=isset($group_parent)?$this->input->post('insert_group_parent'):$this->client['client_grpid'];
                        
                        $client_id=$_SESSION['client']['client_id'];
                        
                        $cgrp_data=array('cgrp_name'=>$group_name,
                                         'cgrp_pid'=>$group_parent,
                                         'cgrp_client_fk'=>$client_id,
                                         'cgrp_desc'=>$group_desc
                            );
                        
                        #print_r($cgrp_data); die;
                        
                         if($this->db->insert('client_groups',$cgrp_data)){
                             // Save any public or admin status or error messages to CI's flash session data.
                            $this->session->set_flashdata('msg', 'group inserted successfully');
                          }else{
                             $this->session->set_flashdata('msg', 'group insertion error');   
                             return false;
                         ...
                         //  
			// Redirect user.
			redirect($this->base_url.'auth_admin/manage_user_groups');			
		}
	}
        
	function insert_user_group_1()
	{
...
//                $parent_group_id=$current_group['ugrp_pid'];
		...
		if ($this->form_validation->run())
		{
//                    echo "validation paas"; 
//                    echo $this->base_url.'auth_admin/manage_user_groups';
//                    echo $this->flexi_auth->get_messages();
                    //die;
			// Get user group data from input.
			$group_name = $this->input->post('insert_group_name');
			...
			$this->flexi_auth->insert_group($group_name,$group_desc,$group_admin,array('ugrp_pid'=>$group_parent,'ugrp_client_fk'=>$client_id));
        ...
        function update_user_group($group_id)
	{
		...
		$validation_rules = array(
			...
			array('field' => 'update_group_parent', 'label' => 'Parent Group', 'rules' => 'integer')
		);
		...
		if ($this->form_validation->run())
		{
                    ...
                    // Get user group data from input.
			 $group_name = $this->input->post('update_group_name');
                         $group_desc = $this->input->post('update_group_description');
                         $group_parent = $this->input->post('update_group_parent'); 
                        ...
                        $this->db->where('cgrp_id',$group_id);
                        $update_result= $this->db->update('client_groups',$cgrp_data);
                    
                        //echo $this->db->last_query();
                        //die();
                        if($update_result){
			$this->session->set_flashdata('msg',"User Group upadted successfully");
			// Redirect user.
			redirect($this->client['url_slug'].'/auth_admin/manage_user_groups');
                        }
	...
	function update_user_group_1($group_id)
	{
		$this->load->library('form_validation');
		...
		if ($this->form_validation->run())
		{
		     // validate if current group has the child then no updation occures 
                    $childs=$this->flexi_auth->get_groups_array("",array('ugrp_pid'=>$group_id));
                    #echo  $this->db->last_query();
                    #print_r($childs);
                    #die;
                    
                     if(count($childs)>0 && 0){
                         $this->session->set_flashdata('message',"this group has the child groups, so cant be updated!");
                         $this->session->set_flashdata('msg',"this group has the child groups, so cant be updated!");
                         redirect($this->client['url_slug'].'/auth_admin/manage_user_groups');
                     }else{
                         
			// Get user group data from input.
			$is_admin = (isset($_POST['update_group_admin']))? $this->input->post('update_group_admin') : '';
			$data = array(
				...
				//$this->flexi_auth->db_column('user_group', 'admin') => $is_admin,
                                'ugrp_pid' => $this->input->post('update_group_parent')
			);
                        #echo"<pre>";
                        #print_r($data);
	...
	{
		$this->load->library('form_validation');
                #$client_id=$this->client['group_id'];  // current loggend in client group id 
                $client_id=$this->client['client_id'];  // current loggend in client group id 
		...
		if ($this->form_validation->run())
		{
			...
			$privilege_desc = $this->input->post('insert_privilege_description');
                        $custom_data= array('upriv_client_fk'=>$client_id);

			$this->flexi_auth->insert_privilege($privilege_name,$privilege_desc,$custom_data);
				
			// Save any public or admin status or error messages to CI's flash session data.
			$this->session->set_flashdata('msg', $this->flexi_auth->get_messages());
			
			// Redirect user.
			redirect($this->base_url.'auth_admin/manage_privileges');			
		}
	}
