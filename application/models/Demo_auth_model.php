...
class Demo_auth_model extends CI_Model {
	...
	private $client;
        ...
        function __construct() 
        {
          ...
          $this->_img_path="./uploads/{$_SESSION['client']['client_id']}/images/users/";
        }
...
//		        $default_group_id =
//                        (isset($_SESSION['user_from']) && $_SESSION['user_from']=='infonet')? '11' : '1';
                        
                        $default_group_id=3;  // default public account 
                        
                        //$default_group_id= $_SESSION['client']['group_id'];                     
                        $profile_data['uacc_client_fk']=$_SESSION['client']['client_id'];
                        $profile_data['uacc_cgroup_fk']=$_SESSION['client']['group_id'];
			...
			if ($response){
                ...
                // Set validation errors.
                $this->data['msg']= $this->data['message'] = validation_errors();
	...
	{	
		...
		{					
			...
			$this->session->set_flashdata('msg', $this->flexi_auth->get_messages());
                        ...
                        redirect($this->client_url.'auth/resend_activation_token');
		...
		if ($this->form_validation->run())
		{
			...
			/* User Profile Image Section edited by Shakti Singh */
				...
				if(isset($_FILES) && !empty($_FILES['user_image']['name'])){
					...
					if($profile_image != ''){
                                            $old_pic_path = "./uploads/{$client_id}/images/users/".$profile_image;
			...
			// die;
			if(isset($_SESSION['user_from']) && $_SESSION['user_from']=='infonet'){
				($response) ? redirect('Infonet') : redirect('auth_public/update_account');
			}else{
				($response) ? redirect($this->client_url.'auth') : redirect('auth_public/update_account');
			}
		}
		else
		{
...
/*
	...
	*/
	...
	public function image_resize($file_name){
		...
		$thumbPath = $this->_img_path.$file_name;
