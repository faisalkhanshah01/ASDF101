...
//class Asm_api extends CI_Controller
{
   ...
   function assign_to_store_v1_post($products){
       
       $products=$this->post('products');
       #$products='[{"mdata_id":"128","product_id":"ASDF128","user_id":"159","group_id":"13"},{"mdata_id":"130","product_id":"ASDF130","user_id":"159","group_id":"13"},{"mdata_id":"132","product_id":"ASDF132","user_id":"159","group_id":13}]';
       ...
       if(!empty($products)){
           ...
           foreach($products as $product){
            $pro_mdata=$this->asm_model->get_data_row('master_data',array('mdata_id'=>$product['mdata_id'])); 
            if($pro_mdata){
                $product['mdata_uin']=$pro_mdata['mdata_uin'];
            }
             $result[]= $this->_assign_to_store($product); 
           }
           
         $response=array('status_code'=>200,'message'=>'Product add to ASM store!','multi_message'=>$result);  
       }else{
           $response=array('status_code'=>400,'message'=>'Bad Request');
       }
   ...
   function _assign_to_store($product){
        
        $mdata_id   = $product['mdata_id'];
        $product_id = $product['product_id']; 
        $user_id    = $product['user_id'];
        $group_id   = $product['group_id'];
        $mdata_uin  = $product['mdata_uin']; 
        ...
        if($this->_verifyASMProduct($mdata_id)){
                return $message="product {$mdata_uin} already available to ASM store";
        }else{
           ...
           if($ps_id){
               ...
               return $message="Product {$mdata_uin} add to ASM store!";  
           }
        }
   }  
   ...
   function store_count_get($store_id){
      $store_id=$this->get('store_id');
        if(!$store_id){
            ...
            $post_data['pending_product']=$this->asm_model->row_count('asm_products',
                                                        array('ps_store_id'=>$store_id,
                                                              'ps_isused'=>2)
                                                       );
            $post_data['rejected_product']=$this->asm_model->row_count('asm_products',
                                                        array('ps_store_id'=>$store_id,
                                                              'ps_isused'=>9)
                                                       );


            #$post_data['free_product']=$post_data['total_product']-$post_data['used_product']-$post_data['pending_product']-$post_data['rejected_product'];
             $post_data['free_product']=$post_data['total_product']-$post_data['used_product']-$post_data['rejected_product'];
            #print_r($post_data);
           ...
           if($filter=='free'){
            ...
            # die;
             ...
             foreach($products as $product){
                 ...
                 if($product_detail['component_imagepath']){
                   $product_detail['component_imagepath']=  str_replace("./",base_url(), $product_detail['component_imagepath']);  
                 }
                 
                if($product['ps_mdata_id']){
                     $pMdata=$this->asm_model->get_data_row('master_data',array('mdata_id'=>$product['ps_mdata_id']));
                      if($pMdata){
                           #print_r($pMdata); die("===========");
                          $productUin=$pMdata['mdata_uin'];
                      }
                 }
                 if($product_detail){

                    $product_detail['mdata_id']= $product['ps_mdata_id'];
                    $product_detail['mdata_uin']= $productUin;
                ...
                if($product['ps_store_id']==$store_id && $product['checkedin']==0){
                    ...
                    $result = $this->asm_model->delete_record('asm_user_project_products',$where);
                      if($result){
                        ...
                        $date=date('d/m/Y');
                        $tdata['pt_comment'] = 'product is return to store'. date('d/m/Y') .".and removed from user projects";
   ...
   # 
     ...
     function assign_to_project_post(){
         ...
         $device_token='fpi4TMQEVJo:APA91bHGcZNIwK3D5A3sI6_e4vkV1q01i7JHyvUThjhy0iW2SZM9r9wBNOqbjpiUgr84QpAhXcb204vww7SSudSdiJCNO8b1A2exBBhua2hJvNJl5byGI-7eA3QqdnxXOMZ9ZsJohBs_';
           ...
           if($this->_verifyASMProduct($mdata_id,$user_id)){

                #echo "p verified"; die; 
                 // here user id  is the receiver id 
                    if($this->_u2u_tansfer_request($mdata_id,$user_id)){  
                        
                       #$this->_u2u_transfer($mdata_id,$user_id,$project_id);
                         ...
                         $receiver_id=$user_id;
                         $this->_u2u_transfer_notification($mdata_id,$store_id,$owner_id,$receiver_id,$project_id);
                         return true;
                         
                    }else{
                       $this->_assign_to_project($mdata_id,$product_id,$user_id,$project_id);
                       return true;				
                    }
    ...
    function _assign_to_project($mdata_id,$product_id,$user_id,$project_id){
        ...
        $dbdata['upp_user_id']=$user_id;
            ...
            #$product=$this->asm_model->get_data('asm_user_project_products',array('upp_mdata_id'=>$mdata_id,'upp_user_id'=>$user_id,'upp_project_id'=>$project_id));
            $product=$this->asm_model->get_data('asm_user_project_products',array('upp_mdata_id'=>$mdata_id));  
            if(!$product){
              ...
              $response=array("status_code"=>404,'message'=>'Product is already available/assigned to Project');   
           }
       #print_r($response); die;
       $this->response($response,200);
    } 
  ...
  function _u2u_tansfer_request($mdata_id,$user_id){
       ...
       if( ($product['ps_user_id'] !=0 ) && ( $product['ps_user_id'] != $user_id) ){
           //echo "U2U transfer request";
             if($product['ps_checkedin']==0 && $product['ps_checkedout']==0){
                   return 1;
             }
            if($product['ps_checkedin']==1 && $product['ps_checkedout']!=1){
                $response=array("status_code"=>404,'message'=>'product not trnsfer- first checkout the product');
                $this->response($response,200);
                exit();
            }
           return 1;
  ...
  function _u2u_transfer($mdata_id,$user_id,$project_id){
  ...
  function _u2u_transfer_notification($mdata_id,$store_id,$owner_id,$receiver_id,$project_id){ 
      
        // get the store token 
       $user=$this->asm_model->get_data_row('user_accounts',array('uacc_id'=>$store_id));
       $token=$user['uacc_device_token']; 
       $device_type=$user['uacc_device_type'];

       //product info  
       $product=$this->asm_model->get_data_row('asm_products',array('ps_user_id'=>$owner_id,'ps_mdata_id'=>$mdata_id));
       $product_code=$product['ps_product_id'];

       //$token='fpi4TMQEVJo:APA91bHGcZNIwK3D5A3sI6_e4vkV1q01i7JHyvUThjhy0iW2SZM9r9wBNOqbjpiUgr84QpAhXcb204vww7SSudSdiJCNO8b1A2exBBhua2hJvNJl5byGI-7eA3QqdnxXOMZ9ZsJohBs_';
       if(!$token){
         $response=array("status_code"=>400,'message'=>'store Device token is not set');   
       }else{
        $owner=$this->_asm_get_username($owner_id);
        $receiver=$this->_asm_get_username($receiver_id);
        $title="U 2 U transfer Notification";
        $msg="{$receiver} is requesting for the approval of {$product_code} from {$owner}.Please Responde with following.";

        $data=array();
        $data['message']=$msg;
        $data['mdata_id']=$mdata_id;
        $data['owner_id']=$owner_id;
        $data['receiver_id']=$receiver_id;
        $data['project_id']=$project_id;  // receiver project id 
        $data['title']=$title;
        $data['timestamp']=time();
        #print_r($data); die;
         if($device_type == 'ios')
         {
          $status = $this->asm_model->ios_notification($token,$title,$msg,$data);  
         }else{
          $status=$this->asm_model->android_notification($token,$title,$msg,$data); 
        }   
        ...
        if($status['success']==1){
             ...
             $this->asm_model->update_table('asm_products',array('ps_isused'=>2),array('ps_mdata_id'=>$mdata_id,'ps_user_id'=>$owner_id)); 
  ...
  function  u2u_transfer_status_post(){
      
        $command=$this->post('u2u_response');
        $mdata_id=$this->post('mdata_id');
        $sender_id=$this->post('u2u_sender');
        $receiver_id=$this->post('u2u_receiver');

        $store_id=$this->post('store_id');  // if other store user scan the product 
        $project_id=$this->post('project_id');
        ...
        if($command=='APPROVED'){
             ...
             $dbdata['ps_user_id']=$receiver_id;
             // update master table 
              $rs= $this->asm_model->update_table('asm_products',array('ps_mdata_id'=>$mdata_id,'ps_user_id'=>$receiver_id,'ps_project_id'=>$project_id,'ps_isused'=>1),
             array('ps_user_id'=>$sender_id,'ps_mdata_id'=>$mdata_id));

             if($rs){
                 //echo "============"; die; 
                 $product=$this->asm_model->get_data_row('asm_products',array('ps_user_id'=>$receiver_id,'ps_mdata_id'=>$mdata_id,'ps_project_id'=>$project_id,'ps_isused'=>1));

                 //print_r($product); die; 

                 $product_id=$product['ps_product_id'];
                 // insert for new 
                 $receiver_data=array('upp_user_id'=>$receiver_id,'upp_mdata_id'=>$mdata_id,'upp_product_id'=>$product_id,'upp_project_id'=>$project_id);
                 $this->asm_model->insert_data('asm_user_project_products',$receiver_data);

                // delete for old 
                $this->asm_model->delete_record('asm_user_project_products',array('upp_mdata_id'=>$mdata_id,'upp_product_id'=>$product_id,'upp_user_id'=>$sender_id)); 
             }
            
              //echo $receiver_id; die; 
             /* $sender=$this->asm_model->get_data_row('user_accounts',array('uacc_id'=>$sender_id));
                $receiver=$this->asm_model->get_data_row('user_accounts',array('uacc_id'=>$sender_id));

              $msg='Product is successfully approved and transfered to user account';
              $title='Product is successfully approved and transfered';

              $sender_token=$sender['uacc_device_token'];
              if($sender_token){
                   $status=$this->asm_model->android_notification($sender_token,$title,$msg);   
              }
              $receiver_token=$receiver['uacc_device_token'];
              if($receiver_token){
                  $status=$this->asm_model->android_notification($receiver_token,$title,$msg); 
              }*/

             if($rs){
                 $response=array('status_code'=>200,'message'=>'Product is successfully approved and transfered to user account');
             }else{
                 $response=array('status_code'=>200,'message'=>'Product is transfered after approval from Store!');
             }
...
function _httpGet($url)
{
           ...
           $response=array('status_code'=>400,'message'=>'user_id parameter is missing !'); 
            ...
            #print_r($products); die; 
             ...
             foreach($products as $product){
                ...
                if($product['upp_mdata_id']){
                    $product_detail['mdata_id']=$product['upp_mdata_id']; 
                    $pMdata=$this->asm_model->get_data_row('master_data',array('mdata_id'=>$product['upp_mdata_id']));		
                    if($pMdata){		
                        $productUin=$pMdata['mdata_uin']; 
                    }
                }
                ...
                $asm_detail=$this->asm_model->get_data_row('asm_products',array('ps_product_id'=>$product['upp_product_id'],'ps_mdata_id'=>$product['upp_mdata_id']));
    ...
    function change_product_status_post(){
        ...
        $params=array('mdata_id','product_id','user_id','action','time');
        ...
        if($this->auth_params($params)){
            ...
            if($action =='checkin'){
              ...
              $checkout_reason=array('reason_abbrev'=>$reason,'reason'=>$reason_txt,'reason_detail'=>$reason_detail);
              $dbdata['pu_reason']= json_encode($checkout_reason);
			  
			        $pu_id=$this->asm_model->update_table('asm_product_usecase',$dbdata,
                                  array('pu_user_id'=>$user_id,'pu_product_id'=>$product_id,'pu_mdata_id'=>$mdata_id));
			  
			  //get the product log hours
			  $log_where=array('pu_user_id'=>$user_id,'pu_product_id'=>$product_id,'pu_checkout'=>$time,'pu_mdata_id'=>$mdata_id);
