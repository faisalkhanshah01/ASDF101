<?php
/*
Below function implement in each place 
				/// Start care fully
	  ...
	  public function update_profile_post() {
        if (isset($_POST) && !empty($_REQUEST['user_id'])) {
			...
			if(!empty($_POST['company_name'])){
				$data['upro_company'] = $_POST['company_name'];
		    }		
...
/* http://192.168.1.3/Mysites/karam/kare/api_controller/components/id or code */
		...
		if($mdata && !empty($mdata[0])){
			...
			/* Images */
				...
				if($val['mdata_asset'] != null ){
					...
					$item_arrays2 = implode('##',$item_array);
				...
				if( $mdata_item_seriesFlag == 1 ){
					$mdata[$key]['mdata_item_series'] = $item_arrays1;  
				}else if( $$mdata_assetFlag == 1) {
					$mdata[$key]['mdata_item_series'] = $item_arrays2;  
				}else{
					$mdata[$key]['mdata_item_series'] = '';  
				}
	...
	*/
	...
	function reportCertificateView_get(){
		...
		$data = $this->_get_report_certificate_data($ref_id,$serial_id, $batch_no, $manufacture_date);		
		$this->load->view('admin_inspection/report_certificate_view', $data);
	}

	function _get_report_certificate_data($ref_id,$serial_id, $batch_no, $manufacture_date){
		$this->load->model('kare_model');
	...
	function _get_asset_component_get( $componentArray ){
		...
		$count=0;
			foreach($componentArray AS $key1){ 
				...
				$tempAsset[$count]['component_code']		= $components['component_code'];
				$tempAsset[$count]['component_description']	= $components['component_description'];											
				...
				$tempAsset[$count]['component_uom']			= $components['component_uom'];
				 $this->load->model('Subassets_model');
