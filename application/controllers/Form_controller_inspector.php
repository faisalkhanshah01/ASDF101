...
class Form_controller_inspector extends CI_Controller{
   ...
   // Initializing kare controller 
	...
	function _work_permit_list($siteID){
		if(!empty($siteID)){
			$data = array();
			$this->load->model('Form_model');
			$work_no = $this->Form_model->check_work_permit_number($siteID);
			...
			$site_array = $this->Form_model->get_siteID_data($siteID);
			$job_card 	= $site_array['site_jobcard'];
			$sms_no 	= $site_array['site_sms'];
			$master_table_details = $data['master_table_details']=$this->Form_model->master_table_details($job_card,$sms_no);
			if(!empty($master_table_details)){
				$asset_series 	= (!empty($master_table_details['mdata_item_series']))?json_decode($master_table_details['mdata_item_series']) : '';
				$data['list']= array(
							'siteID_id' 		=> $siteID,
	...
	/*function insert_workPermit_report(){
		...
		// Delete Old Image
		if(isset($_SESSION['inspector']['workPermit']['ins_image']) && $_SESSION['inspector']['workPermit']['ins_image']!=''){
			$old_insimage 	= $_SESSION['inspector']['workPermit']['ins_image'];
		}
		
		
		if(isset($_FILES['inspector_image'])){
			if($_FILES['inspector_image']['name']!=''){
					$field_name 		= 'inspector_image';
					$folder_path 	= 	"./uploads/inspector_image/";
					...
					if(isset($data['error'])){
					   ...
					   redirect('Form_controller/work_permit_form', 'refresh');
					}else{
						$ins_image	=	$data['file_path'];
					}
			}else{
				$ins_image = '';
			}
		...
		$data= array(
					...
					'batch_no'						=>	!empty($_SESSION['inspector']['workPermit_batch_no'])?$_SESSION['inspector']['workPermit_batch_no']:$this->input->post('batch_no'),
					'serial_no'						=>	!empty($_SESSION['inspector']['workPermit_serial_no'])?$_SESSION['inspector']['workPermit_serial_no']:$this->input->post('serial_no'),
		...
		if(isset($_SESSION['inspector']['form'])){
			if(isset($old_insimage)){
				$path = str_replace('FCPATH','.', $old_insimage);
				unlink($path);
			}
	...
	function work_permit_form_inspection_new(){
		if(!empty($_REQUEST['submit']) && !empty($_REQUEST['siteID'])){
			$param = array();
			$_SESSION['inspector']['form_submitting_date'] = $_REQUEST['today_date'];
			
			$inspection_listID = $this->Form_model->get_inspection_list_1_temp($_REQUEST['siteID']);
			$inspectionlistID = !empty($inspection_listID['id'])?$inspection_listID['id']:'';
			$_SESSION['inspector']['temp1_return_id'] = $inspection_listID['id'];
			$returnID = $this->insert_inspection_list_1_temp_new($inspectionlistID);
				
			$param['siteId_name'] = $returnID;
			$param['site_id'] = $_REQUEST['siteID'];
			
			if(!empty($_REQUEST['work_permit_client_name'])){
                $param['client_name'] = $_REQUEST['work_permit_client_name'];
			}
			if(!empty($_REQUEST['today_date'])){
					$param['today_date'] = $_REQUEST['today_date'];
			}
			if(!empty($_REQUEST['work_permit_workPermit_number'])){
					$param['workPermit_number'] = $_REQUEST['work_permit_workPermit_number'];
			}
			if(!empty($_REQUEST['permitDate_from'])){
                $param['permitDate_from'] = $_REQUEST['permitDate_from'];
			}
			if(!empty($_REQUEST['permitValid_upto'])){
					$param['permitValid_upto'] = $_REQUEST['permitValid_upto'];
			}
			if(!empty($_REQUEST['work_permit_siteID_address'])){
					$param['siteID_address'] = $_REQUEST['work_permit_siteID_address'];
			}
			if(!empty($_REQUEST['workPermit_lattitude'])){
                $param['gprs_lattitude'] = $_REQUEST['workPermit_lattitude'];
			}
			if(!empty($_REQUEST['workPermit_longitude'])){
					$param['gprs_longitude'] = $_REQUEST['workPermit_longitude'];
			}
			if(!empty($_REQUEST['work_permit_workPermit_number'])){
					$param['workPermit_number'] = $_REQUEST['work_permit_workPermit_number'];
			}
			if(!empty($_REQUEST['workPermit_asset_series'])){
                $param['asset_series'] = $_REQUEST['workPermit_asset_series'];
			}
			if(!empty($_REQUEST['batch_no'])){
					$param['batch_no'] = $_REQUEST['batch_no'];
			}
			if(!empty($_REQUEST['serial_no'])){
					$param['serial_no'] = $_REQUEST['serial_no'];
			}
			if(!empty($_REQUEST['harness'])){
                $param['harness'] = $_REQUEST['harness'];
			}
			if(!empty($_REQUEST['helmet'])){
					$param['helmet'] = $_REQUEST['helmet'];
			}
			if(!empty($_REQUEST['shoes'])){
					$param['shoes'] = $_REQUEST['shoes'];
			}
			if(!empty($_REQUEST['gloves'])){
                $param['gloves'] = $_REQUEST['gloves'];
			}
			if(!empty($_REQUEST['goggle'])){
					$param['goggle'] = $_REQUEST['goggle'];
			}
			if(!empty($_REQUEST['work_position'])){
					$param['work_position'] = $_REQUEST['work_position'];
			}
			if(!empty($_REQUEST['equipment_use'])){
                $param['equipment_use'] = $_REQUEST['equipment_use'];
			}
			if(!empty($_REQUEST['worker_height'])){
					$param['worker_height'] = $_REQUEST['worker_height'];
			}
			if(!empty($_REQUEST['weather_good'])){
					$param['weather_good'] = $_REQUEST['weather_good'];
			}
			if(!empty($_REQUEST['unguarded_edges'])){
                $param['unguarded_edges'] = $_REQUEST['unguarded_edges'];
			}
			if(!empty($_REQUEST['equipment_calibrated'])){
					$param['equipment_calibrated'] = $_REQUEST['equipment_calibrated'];
			}
			if(!empty($_REQUEST['physically_fitness'])){
					$param['physically_fitness'] = $_REQUEST['physically_fitness'];
			}
			if(!empty($_REQUEST['alcohol_influence'])){
					$param['alcohol_influence'] = $_REQUEST['alcohol_influence'];
			}
			if(!empty($_REQUEST['mediclaim_insured'])){
                $param['mediclaim_insured'] = $_REQUEST['mediclaim_insured'];
			}
			if(!empty($_REQUEST['client_approval'])){
					$param['client_approval'] = $_REQUEST['client_approval'];
			}
			if(!empty($_REQUEST['documentation_with_client'])){
					$param['documentation_with_client'] = $_REQUEST['documentation_with_client'];
			}
			
			//print_r($param);die;
			$workPermitID = $this->Form_model->get_workPermit($_REQUEST['siteID']);
			if(!empty($workPermitID['site_id']) && !empty($workPermitID['wpermit_id'])){
				$result = $this->Form_model->update_work_permit_new($workPermitID['wpermit_id'], $param);
			}else{
				$result = $this->Form_model->insert_work_permit($param);
			}
			
			if($result > 0){
					$data['responseType'] = 'success';
					$data['site_id'] = $_REQUEST['siteID'];
					$data['wpermit_id'] = !empty($workPermitID['wpermit_id'])?$workPermitID['wpermit_id']:$result;
			} else {
		...
		$data = $this->_work_permit_list($_REQUEST['siteID']);
		$this->load->view('inspector/work_permit_form_inspection_new',$data);	
	...
	function inspector_assign_new(){
		...
		if(is_array($assign_list)){
			foreach($assign_list as $key=>$val){
				...
				$data['assign_list'][$key]['approved_status'] 	= $this->Form_model->get_approved_status($val['siteID_id']);
				$data['assign_list'][$key]['approved_admin_id'] = $this->Form_model->get_admin_approved_status($val['siteID_id']);
				$wpermitID = $this->Form_model->get_workPermit($val['siteID_id']);
				$data['assign_list'][$key]['wpermitID'] = !empty($wpermitID['wpermit_id'])?$wpermitID['wpermit_id']:'';
				$inspection_list = $this->Form_model->get_inspection_list_1_temp($val['siteID_id']);
				$data['assign_list'][$key]['inspection_list'] = !empty($inspection_list)?$inspection_list:'';
			}
		}
		//print_r($data);die("123");
		$this->load->view('inspector/inspector_inspection_list_new',$data);
	}
	...
	*/
	function insert_inspection_list_1_temp_new($inspector_id=''){
		...
		if(!empty($inspector_id)){
			...
			return $this->Form_model->first_slot_data_update($inspector_id,$dbdata);
		}else{
			$dbdata=array(
				'wpermit_id'	=>'',
				'siteID_id'		=>$_SESSION['inspector']['work_permit_siteID_name'],
	...
	function inspection_form_rfid_uin_new(){
		if(!empty($_REQUEST['siteID']) && !empty($_REQUEST['wpermitID'])){
			$id = $_REQUEST['siteID'];
			...
			$master_table_details = $this->data['master_table_details']=$this->Form_model->master_table_details($job_card,$sms_no);
			$wpermitData = $this->Form_model->get_workPermit($_REQUEST['siteID']);
			if(!empty($master_table_details)){
					$_SESSION['inspector']['pono'] 			= $po_no 			= $master_table_details['mdata_po'];
					$_SESSION['inspector']['batch_no'] 		= $batch_no 		= !empty($master_table_details['mdata_batch'])?$master_table_details['mdata_batch']:$wpermitData['batch_no'];
					$_SESSION['inspector']['serial_no'] 	= $serial_no 		= !empty($master_table_details['mdata_serial'])?$master_table_details['mdata_serial']:$wpermitData['serial_no'];
				...
				if($asset_series!=''){
					...
					$array = $this->data['details'] = array(
						...
						'inspector_name' 	 =>	!empty($_SESSION['inspector']['selected_inspector_name'])?$_SESSION['inspector']['selected_inspector_name']:$userdata['firstName'].' '.$userdata['lastName'],
						'site_location'  =>		$site_location,
						'report_no'  	 =>		$_SESSION['inspector']['report_no'],
						'site_city' 	 =>		$site_city
					);
				
					$this->load->view('inspector/inspection_first_form_new', $this->data);
				}else{
	...
	/* //[reportno] => 9889_2017-0001
    //[form_submitting_date] => 09-Nov-2017
    [client_name] => RELIANCE JIO INFOCOMM LIMITED
   // [site_id] => I-GJ-DEES-ENB-0013
   // [job_card] => 13-14/491
    //[sms_no] => PR/13-14/765/TOWER(RELIANCE)/001
    //[asset_series] => TOWER(RELIANCE)
   // [pono] => 118/63006595
    [batchno] => 222222
    [serialno] => 11111
    //[lattitude] => NA
    //[longitude] => NA
   // [Inputvalue] => I10-550000153
   // [inputmethod] => UIN
    [submit] => 1*/
	function inspection_first_form_submit_new(){
		if (!empty($_REQUEST['submit']) && !empty($_REQUEST['wpermitID']) && !empty($_REQUEST['siteID'])){
			/* When Data is Submitted from 1st Form */
			$approvedby		=	'';
			$reportno		=	$_REQUEST['reportno'];
			$siteid			=	$_REQUEST['site_id'];
			$date			=	$_REQUEST['form_submitting_date'];
			$jobcardno		=	$_REQUEST['job_card'];
			$smsno			=	$_REQUEST['sms_no'];
			$asset_series	=	$_REQUEST['asset_series'];
			$pono			=	$_REQUEST['pono'];
			
			$_SESSION['inspector']['inspectedby']	=	$inspectedby	=	$_REQUEST['inspectedby'];
			$_SESSION['inspector']['lattitude']		=	$lattitude		=	$_REQUEST['lattitude'];
			$_SESSION['inspector']['longitude']		=	$longitude		=	$_REQUEST['longitude'];
			$_SESSION['inspector']['inputmethod']	=	$inputmethod	=	$_REQUEST['inputmethod'];
			$_SESSION['inspector']['inputvalue']	=	$Inputvalue		=	$_REQUEST['Inputvalue'];
	 
			$dbdata=array(
				'wpermit_id'	=>$_REQUEST['wpermitID'],
			...
			$inspection_listID = $this->Form_model->get_inspection_list_1_temp($_REQUEST['siteID']);
			$inspectionlistID = !empty($inspection_listID['id'])?$inspection_listID['id']:$_SESSION['inspector']['temp1_return_id'];
		
			if(!empty($inspectionlistID)){
				$result= $this->Form_model->first_slot_data_update($inspectionlistID,$dbdata);
			}else{
			...
			if($result > 0){
					$data['responseType'] = 'success';
					$data['site_id'] = $_REQUEST['siteID'];
					$data['wpermit_id'] = $_REQUEST['wpermitID'];
					$data['inspection_list_1_temp_id'] = $result;
			} else {
					$data = array( 'message' => 'Please try again!');			
			}
			//print_r($data);die("123");
	...
	function inspection_second_form_new(){
		/* When Final CLick button is Clicked*/
		//$_SESSION['inspector']['form_one_inserted_id']
		if(isset($_POST['inspection_second_form'])){
			die("123");
			redirect('form_controller/inspector_information_form_new');
		}
		
		$_SESSION['inspector']['form_one_inserted_id'] = $_REQUEST['inspection_list'];
		$_SESSION['inspector']['form_one_inserted_id_update'] = $_REQUEST['inspection_list'];
		
		$result = $this->Form_model->get_inspection_list_1_temp_id($_REQUEST['inspection_list']);
		$job_card = $result['job_card'];
		$sms_no = $result['sms'];
		
		$master_table_details = $this->data['master_table_details']=$this->Form_model->master_table_details($job_card,$sms_no);
		
		$site_array = $this->Form_model->get_siteID_data($result['siteID_id']);
		$_SESSION['inspector']['siteid'] 		= $site_id 			= ($site_array['site_id']==$result['site_id'])?$result['site_id']:"";
		
		
		$wpermitData = $this->Form_model->get_workPermit($result['siteID_id']);
		$_SESSION['inspector']['form_submitting_date'] = $wpermitData['today_date'];
		if(!empty($master_table_details)){
			$_SESSION['inspector']['pono'] 			= $po_no 			= $master_table_details['mdata_po'];
			$_SESSION['inspector']['batch_no'] 		= $batch_no 		= !empty($master_table_details['mdata_batch'])?$master_table_details['mdata_batch']:$wpermitData['batch_no'];
			$_SESSION['inspector']['serial_no'] 	= $serial_no 		= !empty($master_table_details['mdata_serial'])?$master_table_details['mdata_serial']:$wpermitData['serial_no'];
		...
		//print_r($master_table_details);die("1233");
		$this->load->view('inspector/inspection_second_form_new');
	}
	...
	function inspect_asset_new(){
		...
		if(!isset($_POST['get_inspect_asset'])){
			...
			/* Insert or Update of form three when Values are submitted */
			if($this->insert_inspect_asset_or_subAsset_new()){
				if(!in_array($_SESSION['inspector']['current_asset_id'], $_SESSION['inspector']['completed_assets'])){
					$_SESSION['inspector']['completed_assets'][] = $_SESSION['inspector']['current_asset_id'];
				}
				//$_SESSION['inspector']['completed_assets'][] = $_SESSION['inspector']['current_asset_id'];
				$inserted_id = $_SESSION['inspector']['form_one_inserted_id'];
				$url = 'form_controller/inspection_second_form_new?inspection_list='.$inserted_id;
				redirect($url);
				//print_r($_SESSION['inspector']['inspect_form_third_asset_code']);die("  123");
				//redirect('form_controller/inspection_second_form_new');
			}else{
				$this->session->set_flashdata('msg','<p class="alert alert-danger capital"><b>SYNTAX ERROR!! : </b>Asset not submitted</p>');
				redirect('form_controller/inspection_second_form_new');
			}
	...
	function inspect_form_third_new(){
		...
		if(isset($_POST['inspection_third_form'])){
			...
			$this->load->view('inspector/inspection_third_form_new');
		}
	}
	...
	function inspect_subAsset_new(){
		...
		if(!isset($_POST['get_inspect_subAsset'])){
			...
			$_SESSION['inspector']['subAsset_inspection'] = ($inspectionType !='' && !empty($this->Form_model->get_inspection_value($inspectionType)))? $this->Form_model->get_inspection_value($inspectionType) : '';
				...
				//print_r($obsRes );die("1233");
			$this->load->view('inspector/inspect_subAsset_form_new');
			...
			if($this->insert_inspect_asset_or_subAsset_new()){
				...
				//href="inspect_form_third_new?count_id='.$count.'&asset_id='.$count.'&asset_code='.$key.'"
				$count = $_SESSION['inspector']['inspect_form_third_count_id'];
				$asset_id = $_SESSION['inspector']['inspect_form_third_asset_id'];
				$asset_code = $_SESSION['inspector']['inspect_form_third_asset_code'];
				$url = 'form_controller/inspect_form_third_new?count_id='.$count.'&asset_id='.$asset_id.'&asset_code='.$asset_code;
				redirect($url);
			}else{
	...
	function insert_inspect_asset_or_subAsset_new(){
		...
		$dbdata	=	array(
						...
						'action_proposed'			=>	'',
						'result'					=>	'',
						'asset_id'                  => $_SESSION['inspector']['current_asset_id'],
						'subAsset_id'               => !empty($_REQUEST['count_subAsset_id'])?$_REQUEST['count_subAsset_id']:$_SESSION['inspector']['count_subAsset_id'],
					);
		//print_r($_SESSION);
		//print_r($dbdata);die("123");
		$result	= $this->Form_model->second_slot_data_inserted_new($dbdata);
