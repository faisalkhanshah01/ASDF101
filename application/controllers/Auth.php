...
class Auth extends CI_Controller {
   ...
   function show_phpinfo(){
    ...
    if(count($client_group) && $sflag){
    	  $client_url=$client_slug."/".$service_slug;
          $_SESSION['client']=array(
              'url_slug'=>$client_url,
              'client_name'=>$client_group['cgrp_name'],
              'client_id'=>$client_group['cgrp_client_fk'],
              'group_id'=>$client_group['cgrp_id'],
              'client_grpid'=>$client_group['cgrp_id']
          );
   ...
   function get_client_settings(){
        ...
        $settings= $this->build_model->get_data_row('ar_clients_2018',array('customer_company_name'=>$client_name));
