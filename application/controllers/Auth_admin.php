...
class Auth_admin extends CI_Controller {
    ...
    function __construct() {
       ...
       // echo "client file";
        ...
        if($this->validate_client_slug()){
            ...
            $this->base_url=$_SESSION['client']['url_slug']."/";
            $this->client_url=$_SESSION['client']['url_slug']."/";
        }

        /*
          if( (!empty($_SESSION['user_group'])) && $_SESSION['user_group'] !='2' && $_SESSION['user_group'] !='3' && $_SESSION['user_group'] !='9' && $_SESSION['user_group'] !='11'){
        ...
        //$this->client=$this->uri->segment(1); 
        //$client_name=$this->uri->segment(1); 
    ...
    if(isset($client['url_slug'])){
          ...
          if( ($urlArr[0]==$client_slug) && ($urlArr[1]==$service_slug) ) {
                ...
                #echo "into false";
                $this->session->set_flashdata("message","Not a valid url-session has expired, pls re-login!");
    ...
    function testMail() {
        ...
        $config['smtp_user'] = '101sachin.tyagi@gmail.com';
        $config['smtp_pass'] = 's@ch!n_101@gmail';
        $config['newline'] = "\r\n";
        ...
        $data = array(
            'mail_to' => 'sachin.tyagi455@flashonmind.com',
            'mail_cc' => 'sachin.tyagi@flashonmind.com',
            'mail_subject' => 'test msg',
            'mail_from' => 'sachin.tyagi@flashonmind.ocm',
            //'mail_message'  => $this->load->view('admin/email_templates/admin_customer_invite', $data, true),
            'mail_mesaage' => 'hi it is test mail'
        );
        $this->send_email($data);

        if ($result = $this->send_email($data)) {
            echo 'mailsend';
        } else {
            echo 'mail not send';
        }
        ...
        $email_data = array('identity' => 'sachin.tyagi@gmail.com');
        $this->flexi_auth->send_email('sachin.tyagi@flashonmind.com', 'Test Mail bu AUTH', 'registration_welcome.tpl.php', $email_data);
    ...
    function dashboard_block() {

        //get the all client of the arresto  arresto_root id =1 
        // get All ths subgroups of arresto 

        $sql = "select * from user_groups where ugrp_pid=1";
        $query = $this->db->query($sql);  // return result set 
        if ($query->num_rows() > 0) {
            $result = $query->result_array();
        }


        $sql = "select * from user_accounts as users join user_groups as groups on users.uacc_client_fk=user.uacc_client_fk and users.uacc_group_fk=groups.ugrp_id";

        // get client list 

        $sql = "select * from user_accounts as users join user_groups as gp on users.uacc_client_fk=gp.ugrp_pid and users.uacc_group_fk=gp.ugrp_id where gp.ugrp_pid=1 and gp.ugrp_type='ARRESTO'";
        ...
        //get client list 
        $sql = "select * from user_groups where ugrp_pid=1 and ugrp_type='ARRESTO_CLIENT'";
        // get the detail from user table 
        $sql = "select * from user_groups join user_accounts on user_gropus.ugrp_id=user_accounts.uacc_client_id where ugrp_pid=1 and ugrp_type='ARRESTO_CLIENT'";

        //$sql="select * from user_accounts as users join user_groups as gp on users.uacc_client_fk=gp.ugrp_pid and users.uacc_group_fk=gp.ugrp_id where gp.ugrp_pid=1 and gp.ugrp_type='ARRESTO_CLIENT'"; 
        $query = $this->db->query($sql);  // return result set 
        if ($query->num_rows() > 0) {
            $result = $query->result_array();
        }
        ...
        if ($this->input->post('invite_submit')) {
            ...
            foreach ($mail_to as $value_mail_to) {
                ...
                if ($result) {
                    ...
                    redirect('admin/new_customer_invitation');
                }
            }
        } else {
    ...
    function profile() {
        ...
        if ($this->input->post('update_account')) {
            ...
            $this->demo_auth_model->update_profile();
        }
    ...
    function manage_user_accounts($group_id) {
    ...
    function update_user_account($user_id) {
        ...
        $client_id=$this->client['group_id'];
        ...
        // Get user groups.
        //$this->data['groups'] = $this->flexi_auth->get_groups_array('',array('ugrp_pid'=>$client_id));
    ...
    function insert_user_group(){
        ...
        // get all the sub groups of clients 
        $client_id=$_SESSION['client']['client_id'];
        $client_grpid=$_SESSION['client']['group_id'];
        
        $this->db->select('*');
        $this->db->where(array('cgrp_pid'=>$client_grpid,'cgrp_client_fk'=>$client_id));
       ...
       function update_user_group($group_id) {
        ...
        $this->data['group']=$query->row_array();
    ...
    function insert_user_group_1() {
        ...
        $groups=$this->flexi_auth->get_groups("",array('ugrp_client_fk'=>$client_id))->result_array();
...
//        print_r($groups); die;
    ...
    function manage_privileges() {
        ...
        #echo $client_id=$this->client['client_id']; die;
        $client_id=$this->client['client_id'];
        
        $sql_where=array('upriv_client_fk'=>0);
        $this->flexi_auth->sql_where($sql_where);
        
        $sql_or_where=array('upriv_client_fk'=>$client_id);
        $this->flexi_auth->sql_or_where($sql_or_where);
        
       # $sql_order_by= sql_order_by(column, sort_direction, overwrite_existing)
        
        $this->flexi_auth->sql_order_by('upriv_id','desc');
    ...
    function list_user_status($status = FALSE) {
        ...
        $sql_where[$this->flexi_auth->db_column('user_acc', 'active')] = ($status == 'inactive') ? 0 : 1;
        $client_id=$this->client['group_id'];
        $sql_where['user_accounts.uacc_client_fk']=$client_id;
