...
class Flexi_auth_model extends Flexi_auth_lite_model
{
	...
	{	
		...
		$user = $query->row();
                ...
                if($user->uacc_client_fk==0){
                    $this->load->model('build_model');
                    $client_group = $this->build_model->get_data_row('client_groups',array('cgrp_client_fk'=>$user->uacc_id));
                    #print_r($client_group); echo$_SESSION['client']['client_grpid']; die;  
                    if($client_group){
