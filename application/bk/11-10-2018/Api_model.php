	...
	*/
	...
	public function verify_user_login($identity, $verify_password)
	{
		...
		if($this->verify_password($identity,$verify_password)){
			...
			if( $uacc_active == 0 ){
			   return $error = array('email'=>$identity,'authenticaton'=>0,'user_fullName'=>'', 'userID'=>'','userGroupID'=>'', 'userType'=>'','uacc_client_fk'=>'','error'=>"Please activate your account");	
			   exit;
			}
			$user_fullName = $user['upro_first_name'].' '.$user['upro_last_name'];
			return $user = array('email'=>$identity,'user_fullName'=>$user_fullName, 'userID'=>$user['uacc_id'],'userGroupID'=>$user['uacc_group_fk'], 'userType'=>$user['ugrp_name'],'authenticaton'=>1,'uacc_client_fk'=>$user['uacc_client_fk'],'error'=>'');
		}else{
			//return false;
		return $error = array('email'=>$identity,'authenticaton'=>0,'user_fullName'=>'', 'userID'=>'','userGroupID'=>'', 'userType'=>'','uacc_client_fk'=>'','error'=>"Username or Password  is Incorrect");	
		}
	}
