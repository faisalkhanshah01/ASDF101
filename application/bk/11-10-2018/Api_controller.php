...
{	
	...
	function authenticate_get(){
		...
		$where=array('customer_uacc_fk'=>$client_id);
		$config=$this->asm_model->get_data_row('ar_clients_2018',$where);
		if($config['customer_logo_path']!=''){
			$config['customer_logo_path']=str_replace("./","http://arresto.in/connect/",$config['customer_logo_path']);
		}
        ...
        if($user){
                 ...
                 $token = ($token)?$token:''; 
                 $this->load->model('asm_model'); 
                 $this->asm_model->update_table('user_accounts',array('uacc_device_token'=>$token), array('uacc_id'=>$user['userID']));
	...
	function _getClientConfig($client_id){
						 ...
						 foreach($mdata_assetArry AS $keyy1=>$vall1){
							 ...
							 */
								 ...
								 if( is_array($componentData) && (sizeof($componentData) > 0 ) ){
								 	...
								 	$componentArr['component_imagepath']=str_replace("./", base_url(), $componentArr['component_imagepath']);
