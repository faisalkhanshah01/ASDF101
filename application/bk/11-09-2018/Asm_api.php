   ...
   function store_list_get($store_id,$filter=''){
        ...
        #} else if(!$pfilter){
           ...
           if($filter=='free'){
		    ...
		    $asm_log_hours=$this->_product_log_hours($product['ps_product_id']);
...
//            print_r($products);
		...
		$asm_log_hours=$this->_product_log_hours($product['upp_product_id']); 
		$product_detail['asm_log_hours']=($asm_log_hours)?$asm_log_hours:''; 
                #print_r($product_detail); die; 
                $post_data[]=$product_detail;
   ...
   function  _verifyUSERProduct($user_id,$product_id){
    ...
    function _product_history($user_id,$product_id){
        ...
        foreach($prodcut_history as $item){
            $parse_data=array();
           if($item['pu_checkin']!=''){
                $parse_data['pu_checkin']=date('Y-m-d H:i:s',$item['pu_checkin']);
            }
            if($item['pu_checkout']!=''){
                $parse_data['pu_checkout']=date('Y-m-d H:i:s',$item['pu_checkout']); 
            }
