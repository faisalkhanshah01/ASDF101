...
//class Asm_api extends CI_Controller
{
   ...
   function store_count_get($store_id){
      $store_id=$this->get('store_id');
        if(!$store_id){
            ...
            $post_data['free_product']=$post_data['total_product']-$post_data['used_product']-$post_data['pending_product'];
            #print_r($post_data);
  ...
  function  u2u_transfer_status_get(){
        ...
        $sender_id=$this->get('u2u_owner');
        $receiver_id=$this->get('u2u_receiver');

        $store_id=$this->get('store_id');  // if other store user scan the product 
        $project_id=$this->get('project_id');


        if($command=='APPROVED'){
             $dbdata['ps_user_id']=$receiver_id;
             $rs= $this->asm_model->update_table('asm_products',array('ps_user_id'=>$receiver_id,'ps_project_id'=>$project_id,'ps_isused'=>0),
              array('ps_user_id'=>$sender_id,'mdata_id'=>$mdata_id));
              ...
              $msg='Product is successfully approved and transfed to user account';
              $title='Product is successfully approved and transfed';
             ...
             if($rs){
                 $response=array('status_code'=>200,'message'=>'Product is transfed after approval from Store!');
             }else{
                 $response=array('status_code'=>200,'message'=>'Product is transfed after approval from Store!');
             }
