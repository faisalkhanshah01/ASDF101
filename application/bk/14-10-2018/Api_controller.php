...
/* http://192.168.1.3/Mysites/karam/kare/api_controller/components/id or code */
	...
	function subassets_get($id=NULL){
            $client_id=$this->get('client_id');
                ...
                if(!$client_id){
                   $response=array('status_code'=>400,'message'=>'Bad Request parameter missing - client_id missing');     
                }else{
		...
		if(!empty($sub_assets[0])){
			foreach($sub_assets as $key=>$val){
				...
				if(is_array($featuredData)){
                   ...
                   $response=array('status_code'=>200,'message'=>'result found','data'=>$sub_assets);     
                }
                else{
                    $response=array('status_code'=>404,'message'=>'No result found'); 
                }
            }
			...
			foreach($components as $key=>$val){
				...
				if($val['component_observation']!=''){
					...
					$components[$key]['component_observation'] = implode('##',array_filter($obs_array));
                        ...
                        $response=array('status_code'=>200,'message'=>'result found','data'=>$components);
		   } else{
