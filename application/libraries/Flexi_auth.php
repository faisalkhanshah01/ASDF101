...
class Flexi_auth extends Flexi_auth_lite
{
	...
	public function login($identity = FALSE, $password = FALSE, $remember_user = FALSE) 
	{
		if ($this->CI->flexi_auth_model->login($identity, $password, $remember_user))
		{
			...
			if(($groupID =='10' || $groupID =='11') && (!isset($_SESSION['user_from']))){
				$_SESSION['user_groupID_kare'] = $groupID;
				$this->CI->flexi_auth_model->set_error_message('login_unsuccessful', 'config');
			}else{
				$this->CI->flexi_auth_model->set_status_message('login_successful', 'config');
			}
	...
	function user_name($userID){
		$username = $this->CI->flexi_auth_model->get_user_name($userID);
	...
	public function resend_activation_token($identity) 
	{
		...
		else if ($this->CI->flexi_auth_model->deactivate_user($user_id))
		{
			...
			/*$user_data = array(
				'user_id' => $user_id,     // change by shakti 'id' => $user_id,
				'identity' => $identity,
		...
		//$template = $this->CI->auth->email_settings['email_template_directory'].$this->CI->auth->email_settings['email_template_activate'];
			...
			if(isset($_SESSION['user_from']) && $_SESSION['user_from']=='infonet'){
				$email_title = ' - Infonet Account Activation';
				$template = $this->CI->auth->email_settings['email_template_directory'].$this->CI->auth->email_settings['infonet_email_template_activate_resend_account'];
			}else{
				$email_title = ' - Account Activation';
				$template = $this->CI->auth->email_settings['email_template_directory'].$this->CI->auth->email_settings['email_template_activate'];
			}
			
			$this->CI->load->library('email/send_mail');
			$email = $this->CI->send_mail->send_register($email_to, $email_title, $user_data, $template);
	...
	public function forgotten_password($identifier) 
	{
		...
		if ($this->CI->flexi_auth_model->forgotten_password($identity))
		{
			...
			$forgotten_password_token = $user->{$this->CI->auth->database_config['user_acc']['columns']['forgot_password_token']}; 
                        ...
                        echo $clint_slug=$this->get_client_slug($user_id);
			...
			$user_data = array(
				...
				'forgotten_password_token' => $forgotten_password_token,
				'user_name'	=> $this->user_name($user_id),
                                'client_url'=>$client_slug
				//'user_name'	=> $user->upro_first_name.' '.$user->upro_last_name
			);
			
			if(isset($_SESSION['user_from']) && $_SESSION['user_from']=='infonet'){
				$email_title = ' - Infonet Password Reset Link';
				$template = $this->CI->auth->email_settings['email_template_directory'].$this->CI->auth->email_settings['infonet_email_template_forgot_password'];
			}else{
				$email_title = ' - Forgotten Password Reset Link';
				$template = $this->CI->auth->email_settings['email_template_directory'].$this->CI->auth->email_settings['email_template_forgot_password'];
			}
        ...
        function get_client_slug($user_id){
            $user = $this->CI->flexi_auth_model->get_users(null, array('uacc_id'=>$user_id))->row_array();
            
            #print_r($user);
            
            if($user['uacc_client_fk']){
                $this->db->where('customer_uacc_fk',$user['uacc_client_fk']);
                $query=$this->db->get('ar_clients_2018');
                $result=$query->row_array();
              }else{
                $this->db->where('customer_uacc_fk',$user['uacc_id']);
                $query=$this->db->get('ar_clients_2018');
                $result=$query->row_array(); 
              }
            if($result){
                  return $result['customer_company_slug'];
            }else{
                return false;
            }
...
//            echo "<pre>";
		...
		if ($user_id)
		{
			...
			if (!empty($activation_token))
			{
				...
				if($user_data['emp_id']!=''){
					$userName = $this->user_name($user_id);
					$this->_kdManger_mail($user_data['emp_id'],$user_id,$identity,$userName);
					$email_title = ' - Infonet Account Activation';
					$email_check = 2;
					$template = $this->CI->auth->email_settings['email_template_directory'].$this->CI->auth->email_settings['infonet_email_template_activate'];
				}else{
					$email_title = ' - Account Activation';
					$email_check = 1;
					$template = $this->CI->auth->email_settings['email_template_directory'].$this->CI->auth->email_settings['email_template_activate'];
	...
	function _kdManger_mail($emp_id,$user_id,$identity,$username){
		if(!empty($emp_id)){
			$kdMangerID = 11;
			$mailID =  $this->CI->flexi_auth_model->fetch_kdmanger_mailId($kdMangerID);
			
			$email_to = 'dhanirams@karam.in';
			//$email_to = 'basant.kumar@flashonmind.com';
			$email_title = ' - Infonet User New Account';
			$template = $this->CI->auth->email_settings['email_template_directory'].$this->CI->auth->email_settings['infonet_email_template_activate_new_user'];
   ...
   // custom function created by sachin 
 /*function userGroupsTree($edit_id=null,$edit_parent=null){
     $tree_root = $_SESSION['client']['group_id'];
     return  $this->groupTree($tree_root,$edit_id,$edit_parent);
     
 } */    
        
        
    
function groupTree($parent=0,$edit_id=null,$edit_parent=null,&$output=null,$indent=null){
            
           // $groups=$this->get_groups_query("",array("ugrp_pid"=>$parent))->result_array();
            
            $this->CI->db->select('*');
            $this->CI->db->where('cgrp_pid',$parent);
            $query=$this->CI->db->get('client_groups');
...
//            die;
            ...
            foreach($groups as $key=>$group){
                    //if($edit_id == $group['ugrp_id']){
                    if($edit_id == $group['cgrp_id']){
                       $disabled="disabled";
                    } else{
                      $disabled="";
                    }
                    if($edit_parent == $group['cgrp_id'] ){
                        $selected="selected";
                    }else{  $selected=""; } 

                   $output .= "<option {$selected} {$disabled} value='".$group['cgrp_id'] ."'>" . $indent . $group['cgrp_name']. "</option>"; 
                   if($group['cgrp_id'] != $parent){
                             // in case the current category's id is different that $parent
                             // we call our function again with new parameters
                            $this->groupTree($group['cgrp_id'],$edit_id,$edit_parent,$output,$indent . "&nbsp;&nbsp;&nbsp;");
                     }
            } 
...
function groupTableTree($parent=0,$edit_id=null,$edit_parent=null,&$output=null,$indent=null){
    ...
    #echo base_url(); die;
           ...
           # $groups=$this->get_groups_query("",array("cgrp_pid"=>$parent))->result_array();
            $disabled = $selected="";
            $base_url=base_url().$_SESSION['client']['url_slug']."/";
           
            foreach($groups as $key=>$group){
                     
                     $is_admin=($group['cgrp_admin']==1)?'Yes':"No";
                     
                     $id=$group['cgrp_id'];
                
                     $output .= "<tr>";
                     $output .= "<td> <a href='{$base_url}auth_admin/update_user_group/$id'>" . $indent.$group['cgrp_name']. "</a></td>"; 
                     $output .= "<td>". $group['cgrp_desc'] ."</td>"; 
                     
                     //$output .= "<td>".$is_admin."</td>"; 
                     
                     //$output .= "<td><a href='{$base_url}auth_admin/update_group_privileges/$id'>manage</a></td>"; 
                     
                     $output .= "<td> <a href='{$base_url}auth_admin/update_user_group/$id'>Edit</a> || <a href='{$base_url}auth_admin/update_user_group/$id'>Delete</a>   </td>";
                   ...
                   if($group['cgrp_id'] != $parent){
                             // in case the current category's id is different that $parent
                             // we call our function again with new parameters
                            $this->groupTableTree($group['cgrp_id'],$edit_id,$edit_parent,$output,$indent . "&nbsp;&nbsp;&nbsp;");
                     }
            } 
             // return the list of categories
          return $output;
