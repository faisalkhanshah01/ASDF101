                    ...
                    <tbody>
                        <?php
                         foreach($asm_products as $prodcut){
                            if($prodcut['ps_isused']==2){
                                $status='Pending';
                            }
                           if($prodcut['ps_isused']==1){
                                $status='Used';
                            }
                           if($prodcut['ps_isused']==0){
                                $status='Free';
                            }        
                        ?>
                        <tr>
    ...
    $('#asm_user').change(function(){
      ...
      $('#search').click(function(e){
       ...
       if(store ||  (user&&store) || (store && user&& product) ){
          if(store){where.ps_store_id=store;}
          if(user){ where['ps_user_id']=user;}
          if(project){ where['ps_project_id']=project;}           
           postData['where']=where;
                 ...
                 success:function(data){
                     if(data.status_code==200){
                        ...
                        $.each(products,function(index,item){
                            if(item['ps_isused']==2){ status='Pending'; }
                              if(item['ps_isused']==1){ status='Used'; }
                                if(item['ps_isused']==0){ status='Free'; }
                            row +="<tr>";
